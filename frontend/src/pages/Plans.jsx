...
import { Plus, Pencil, Trash2, Layers, Ticket } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1 dark:text-slate-400">Create and manage internet access plans.</p>
        </div>
        <button
          onClick={openAdd}
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {plans.map((plan) => (
                  <tr key={plan.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{plan.duration_minutes} min</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">
                      {plan.speed_down_kbps ? `${plan.speed_down_kbps}↓` : '—'}
                      {plan.speed_up_kbps ? ` / ${plan.speed_up_kbps}↑` : ''}
                      {plan.speed_down_kbps ? ' kbps' : ''}
                    </td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editing ? 'Edit Plan' : 'Add Plan'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Download Speed (kbps)</label>
              <input
                type="number"
                min={0}
                value={form.speedDownKbps}
                onChange={(e) => setForm({ ...form, speedDownKbps: e.target.value ? Number(e.target.value) : '' })}
                className="w-full px-4 py-2 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 dark:bg-slate-950 dark:border-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-600"
                placeholder="e.g. 2048"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Upload Speed (kbps)</label>
              <input
                type="number"
                min={0}
                value={form.speedUpKbps}
                onChange={(e) => setForm({ ...form, speedUpKbps: e.target.value ? Number(e.target.value) : '' })}
                className="w-full px-4 py-2 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 dark:bg-slate-950 dark:border-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-600"
